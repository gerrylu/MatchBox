...
public class NanoHTTPD {
	...
	static List<EALooperTask> m_EATasks = new ArrayList<EALooperTask>();
	
	static public void AddLooperTask(EALooperTask t){
		m_EATasks.add(t);
				
		new Thread(new Runnable() {
			public void run() {
				HttpRequestHelper httpAgent = new HttpRequestHelper();
				String sUrl = "http://127.0.0.1:" + String.valueOf(SERVER_PORT);
				sUrl += "/starttask";
	...
	static public void NotifyLooperTask(){
		...
		myThread = new Thread(new Runnable() {
			public void run() {
				try {
					while (true){
						if (m_EATasks.size() > 0){
							for (int i = 0; i < m_EATasks.size(); ++i){
								EALooperTask t = m_EATasks.get(i);;
								t.run();
							}
							m_EATasks.clear();
						}
	...
	/*
	 * private static final String SERVER_KEY_PASSWORD = "123456"; private
	 * static final String SERVER_AGREEMENT = "TLS";//使用协议 private static final
	 * String SERVER_KEY_MANAGER = "SunX509";//密钥管理器 private static final String
	 * SERVER_KEY_KEYSTORE = "BKS";//密库，这里用的是Java自带密库 private static final
	 * String SERVER_KEYSTORE_PATH = "src/data/kserver.keystore";//密库路径 public
	 * ServerSocket CreateSSlSrvSocket() { ServerSocket sslSock = null; try {
	 * KeyStore sslKeyStore; sslKeyStore =
	 * KeyStore.getInstance(SERVER_KEY_KEYSTORE);
	 * 
	 * InputStream sslKeyData = null;EAUtil.eaContext.getResources()
	 * .openRawResource(R.raw.easslcert);
	 * 
	 * sslKeyStore.load(sslKeyData, SERVER_KEY_PASSWORD.toCharArray());
	 * 
	 * KeyManagerFactory localKeyManagerFactory = KeyManagerFactory
	 * .getInstance(KeyManagerFactory.getDefaultAlgorithm());
	 * 
	 * localKeyManagerFactory.init(sslKeyStore,
	 * SERVER_KEY_PASSWORD.toCharArray()); KeyManager[] arrayOfKeyManager =
	 * localKeyManagerFactory .getKeyManagers(); SSLContext localSSLContext =
	 * SSLContext.getInstance(SERVER_AGREEMENT);
	 * localSSLContext.init(arrayOfKeyManager, null,null);
	 * SSLServerSocketFactory localSSLServerSocketFactory = localSSLContext
	 * .getServerSocketFactory();
	 * 
	 * sslSock = localSSLServerSocketFactory.createServerSocket(myTcpPort);
	 * return sslSock; } catch (Exception e) { // TODO Auto-generated catch
	...
	/*		Thread t = new Thread(this);
		...
		public void run() {
			try {
				...
				byte[] bmethod = new byte[3];
				is.read(bmethod, 0, 3);
				String tmps = new String(bmethod);
				// tmps += "\r\n";
				if (tmps.equalsIgnoreCase("POS")) {
					// guess it's post method
					Response r = servePost(is);
					if (r == null)
				...
				//String uri1 = java.net.URLDecoder.decode(uri);
